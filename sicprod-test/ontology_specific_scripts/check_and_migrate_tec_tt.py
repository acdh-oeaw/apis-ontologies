...
def run(*args, **kwargs):
    ...
    def check_tempentityclass():
        ...
        for tec in TempEntityClass.objects.all():
            ...
            update_field_counter_if_not_none(
                tec, fields_counter_dict, "start_start_date"
            )
            ...
            update_field_counter_if_not_none(
                tec, fields_counter_dict, "start_date_written"
            )
            update_field_counter_if_not_none(
                tec, fields_counter_dict, "end_date_written"
            )
    ...
    def check_temptriple():
        ...
        for tt in TempTriple.objects.all():
            ...
            update_field_counter_if_not_none(
                tt, fields_counter_dict, "start_start_date"
            )
            ...
            update_field_counter_if_not_none(
                tt, fields_counter_dict, "start_date_written"
            )
            update_field_counter_if_not_none(
                tt, fields_counter_dict, "end_date_written"
            )
    ...
    def print_tmp_fields_tec(model_fields_counter_dict, prefix_str):
        for model_class, fields_counter_dict in model_fields_counter_dict.items():
            ...
            for field, count in fields_counter_dict.items():
                print(
                    f"{prefix_str}{field_tmp_mapping[field]}",
                ...
                for field_old in fields_counter_dict.keys():
                    ...
                    if "ManyToManyField" in field_tmp_mapping[field_old]:
                        ...
                        setattr(
                            entity_instance,
                            field_new,
                            getattr(entity_instance, field_old),
                        )
    ...
    def copy_fields_from_temptriple(tt_fields_counter_dict):
        for temptriple_context, fields_counter_dict in tt_fields_counter_dict.items():
            ...
            for temptriple in TempTriple.objects.filter(
                subj__self_contenttype=caching.get_contenttype_of_class(
                    entity_class_subj
                ),
                obj__self_contenttype=caching.get_contenttype_of_class(
                    entity_class_obj
                ),
                prop=prop_instance,
            ):
                # TODO: Replace these if-statements here
