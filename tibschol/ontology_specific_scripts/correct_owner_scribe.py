"""Fix errors introduced by add_owner_scribe"""
import logging
...
def run():
    ...
    for i, row in df.iterrows():
        ...
        if not instance:
            ...
            continue
        bad_scribe = get_person_object(row.AP.strip())
        scribe = get_person_object(row.AQ.strip())
        owner = get_person_object(row.AR.strip())
        if bad_scribe is not None:
            print(i, "Deleted scribe - ", bad_scribe)
            bad_scribe.delete()
        ...
        if scribe is not None:
            ...
            # Find and delete bad owner relationship
            try:
                bad_owner = TempTriple.objects.get(
                    subj=scribe, obj=instance, prop=owner_of
                )
                bad_owner.delete()

            except TempTriple.DoesNotExist:
                print(i, f"Could not find bad owner rel for scribe {scribe}---->")

        if owner is not None:
            print(f"Found owner {owner} at row {i}")
